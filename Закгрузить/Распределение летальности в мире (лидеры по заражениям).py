...
for i in range(4): # Задаём, какое количество стран взять от начала перечня. Они отсортированы по количеству заражённых. Всего стран в перечне на сегодня 145.
    obj1 = obj[i]
    country = obj1['en']
    deaths = obj1['deaths']
    cured = obj1['cured']
...
plt.title('Распределение летальности пациентов с Covid-19 по количеству стран (лидеры по заражениям)') # Если считается летальность, заменить "смертность" на "летальность". Если считается по всем странам, убрать "(лидеры по заражениям)".
